    ...
    public function ajax_list()
    {
        ...
        if($checkColumn == 1){
            ...
            if($tradeType != "" && $bidType != ""){
              ...
              $where = array('DATE(open_order) >=' => $fromDate, 'DATE(open_order) <=' => $toDate, 'status' => $tradeType, 'bid_type' => $bidType, 'trade_type' => NULL);

            } else if($tradeType != "" && $bidType == ""){

              $where = array('DATE(open_order) >=' => $fromDate, 'DATE(open_order) <=' => $toDate, 'status' => $tradeType, 'trade_type' => NULL);

            } else if($bidType != "" && $tradeType == ""){

              $where = array('DATE(open_order) >=' => $fromDate, 'DATE(open_order) <=' => $toDate, 'bid_type' => $bidType, 'trade_type' => NULL);

            } else {

                $where = array('DATE(open_order) >=' => $fromDate, 'DATE(open_order) <=' => $toDate, 'trade_type' => NULL);
            }
    ...
    public function ajax_list_deposit()
    {
        ...
        foreach($list as $depositList){ 
            ...
            if($depositList->status == 0){
                ...
                $statusValue = "<i title=".display('send_gasfee')." class='far fa-times-circle mr-2 fs-20 text-success'></i> ".display('send_gasfee');
