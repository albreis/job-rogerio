...
class Deposit extends BaseController
{
    ...
    */
    ...
    public function payment_gateway(){ 
        ...
        $date           = new \DateTime();
        $deposit_date   = $date->format('Y-m-d H:i:s');
        $method         = $this->request->getVar('method', FILTER_UNSAFE_RAW);  
        $amount = $this->request->getVar('amount', FILTER_UNSAFE_RAW);
        $fees = $this->request->getVar('fees', FILTER_UNSAFE_RAW);
        ...
        if ($this->validation->withRequest($this->request)->run()){
            ...
            $sdata['deposit'] = (object)$userdata = array(
                ...
                'fees'           => $fees,
                'comments'       => $this->request->getVar('comments', FILTER_UNSAFE_RAW),
    ...
    */
    ...
    { 
        ...
        foreach ($list as $deposit) {
            ...
            if($deposit->status==0){
                $row[] = '<i class="fas fa-spinner fa-spin text-warning"></i> '. display('pending');
            }else if($deposit->status==1){
                $row[] = '<i class="fas fa-check text-success"></i>'.  display('send_gasfee');
            }else if($deposit->status==2){
                $row[] = '<i class="fas fa-check text-warning"></i>'.display('fund_received'); 
            }else if($deposit->status==3){
                $row[] = '<i class="fas fa-check text-success"></i>'.display('success');
            }else if($deposit->status==4){
                $row[] = '<i class="fas fa-times text-danger"></i>'.display('canceled');
            }else if($deposit->status==5){
                $row[] = '<div class="progress">
                                  <div title="First Step Completed!" class="progress-bar progress-bar-striped bg-success progress-bar-animated width85" role="progressbar" aria-valuenow="98" aria-valuemin="0" aria-valuemax="100"> '. display('processing').'
                                  </div>
                                </div>'; 
            }else if(($deposit->status==0 || $deposit->status==6) && $deposit->expire_date < date('Y-m-d h:i:s')) {
                $row[] = '<i class="fas fa-calendar-times text-danger"></i>'.display('expired');
            }else{
