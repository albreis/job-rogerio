...
class Withdraw extends BaseController
{
    ...
    public function store()
    {
        ...
        if($this->validation->withRequest($this->request)->run()){
            ...
            $varify_media   = $this->request->getVar('varify_media',FILTER_UNSAFE_RAW);
            $varify_code    = $this->randomID();
            #----------------------------
            // check balance 
            $blance         = $this->check_balance($this->request->getVar('amount',FILTER_UNSAFE_RAW));
            ...
            if($blance!=true){
                ...
                if($code_send != NULL ){
                    ...
                    // get withdraw fees
                    $fees = $this->fees_load($this->request->getVar('amount',FILTER_UNSAFE_RAW),$this->request->getVar('method',FILTER_UNSAFE_RAW),'withdraw');
                    #-----------------------
                    $withdraw = array(
                        'user_id'       => $this->session->get('user_id'),
                        'amount'        => $this->request->getVar('amount',FILTER_UNSAFE_RAW),
                        'fees'          => @$fees,
                        'walletid'      => $this->request->getVar('walletid',FILTER_UNSAFE_RAW),
    ...
    */
    public function withdraw_verify()
    {
        ...
        $code = $this->request->getVar('code',FILTER_UNSAFE_RAW);
        $id = $this->request->getVar('confirm_id',FILTER_UNSAFE_RAW);
