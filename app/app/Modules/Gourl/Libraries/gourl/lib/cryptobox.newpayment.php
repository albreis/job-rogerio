...
function cryptobox_new_payment($paymentID = 0, $payment_details = array(), $box_status = "")
{
	...
	if (isset($payment_details['status']) && @$payment_details['status'] == 'payment_received' && $box_status == "cryptobox_newrecord") {
		$recordExists = run_sql("SELECT comment as nme FROM `dbt_deposit` WHERE comment = '".$paymentID."'");
		...
		if (!$recordExists) {
			...
			run_sql("INSERT INTO `dbt_deposit` VALUES(NULL, NULL, '".$payment_details["user"]."', NULL, '".$payment_details["coinlabel"]."',".(floatval($payment_details["amount"])-floatval($fees)).",'".$paymentID."', NULL, NULL, NULL,NULL,'bitcoin',".floatval($fees).",'".$paymentID."','".gmdate("Y-m-d H:i:s")."','".gmdate("Y-m-d H:i:s")."', NULL,'3','".@$_SERVER['REMOTE_ADDR']."', 'admin')");
