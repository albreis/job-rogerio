...
if(SSS !== 0){
    define('UPDATE_INFO_URL_BDTASK','https://update.bdtask.com/tradebox/autoupdate/update_info');
    //Get Update file
...
class Autoupdate extends BaseController
{
    ...
    public function index(){ 
        ...
        if(SSS === 0){  
            ...
            $data = array();
            $data['latest_version']  = file_get_contents(UPDATE_INFO_URL_BDTASK);
    ...
    public function update()
    {
        ...
        $purchase_key   = $this->request->getVar('purchase_key', FILTER_UNSAFE_RAW);      
        $purchase_key   = trim($purchase_key);
        $latest_version = file_get_contents(UPDATE_INFO_URL_BDTASK);
        $url            = UPDATE_URL;
    ...
    **/
    ...
    private function current_version(){
        ...
        //Current Version
        $product_version = '';
        $path = FCPATH.'vendor/codeigniter4/framework/system/Security/lic.php';
    ...
    public function download() {
        ...
        $response = $this->response->download('./public/assets/data/backup/' . $db_name, NULL);

        $this->session->setFlashdata('message', 'Database backup successful');
