...
class UserController extends BaseController
{
    ...
    public function ajax_list(){
        ...
        foreach ($list as $users) {
          ...
          $row = array();
            ...
            $row[] = $users->email;
            $row[] = $users->phone;
            $row[] = (($users->status==1)?'<a class="btn btn-success btn-sm text-white">'.display('active').'</a>':(($users->status==2)?'<a class="btn btn-danger btn-sm">'.display('pending').'</a>':(($users->status==3)?'<a class="btn btn-danger btn-sm">Suspend</a>':'<a class="btn btn-warning btn-sm text-white">Deactive</a>'))).'  '.(($users->verified==1)?'<a class="btn btn-success btn-sm text-white">'.display('verified').'</a>':(($users->verified==2)?'<a class="btn btn-danger btn-sm text-white">'.display('cancel').'</a>':(($users->verified==3)?'<a href='.base_url("backend/user/pending-user-verification/$users->user_id").' class="btn btn-info btn-sm text-white" data-toggle="tooltip">'.display('requested').'</a>':'<a class="btn btn-danger btn-sm text-white">'.display('not').' '.display('verified').'</a>')));
            $row[] = $emailVerify;
            $row[] = $mobileVerify;
            $row[] = $kycVerify;
    ...
    public function pending_user_verification_list(){
        ...
        $page_number      = (!empty($this->request->getGet('page'))?$this->request->getGet('page'):1);
        $condition        = "verified ='3' OR verified ='13'";
        $data['users']    = $this->common_model->get_all('dbt_user', $condition, 'id','asc',20,($page_number-1)*20);
        $total            = $this->common_model->countRow('dbt_user', $condition);
        $data['pager']    = $this->pager->makeLinks($page_number, 20, $total, 'pagination_template');
    ...
    { 
        ...
        { 
            $data['user'] = (object)$userdata = array(
                ...
                'referral_id' => $this->request->getPost('referral_id', FILTER_UNSAFE_RAW) ?? "",
                ...
                'phone'       => $this->request->getPost('mobile', FILTER_UNSAFE_RAW) ?? "",
            ...
            $data['user'] = (object)$userdata = array(
                'id'          => $this->request->getPost('id', FILTER_UNSAFE_RAW),
                'user_id'     => $this->request->getPost('user_id', FILTER_UNSAFE_RAW),
                'first_name'  => $this->request->getPost('first_name', FILTER_UNSAFE_RAW),
                'last_name'   => $this->request->getPost('last_name', FILTER_UNSAFE_RAW),
                'email'       => $this->request->getPost('email', FILTER_UNSAFE_RAW),
                'language'    => @$dlanguage->language,
    ...
    function ajax_tradelist()
    {
        ...
        $data['pager']    = $this->pager->makeLinks($page_number, 50, $total, 'pagination_template');
