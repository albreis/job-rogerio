...
class Module extends BaseController
{
    ...
    public function upload()
    {
      ...
      if ($this->validation->withRequest($this->request)->run()) {
          ...
          $purchase_key = trim($this->request->getVar('purchase_key',FILTER_UNSAFE_RAW));
          ...
          if(!empty($response) && ($response->status == 'valid')) {
                ...
                $overwrite = $this->request->getVar('overwrite',FILTER_UNSAFE_RAW);
                $overwrite = (($overwrite != null) ? $overwrite : false);
                if($file = $this->request->getFile('module', FILTER_UNSAFE_RAW))
                {
    ...
    public function install()
    {
        ...
        $moduleData = array(
            ...
            'image'         => $this->request->getVar('image',FILTER_UNSAFE_RAW),
            'directory'     => $directory,
            'status'        => 1
        );
        /*-----------------------------------*/
    ...
    public function verify_download_request() {
      ...
      $itemtype = $this->request->getVar('itemtype',FILTER_UNSAFE_RAW);
