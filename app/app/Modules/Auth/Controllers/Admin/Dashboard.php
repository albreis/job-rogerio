...
class Dashboard extends BaseController
{
    ...
    { 
        ...
        if ($this->request->getMethod() == 'post') {
            ...
            $image = '';
            if($img = $this->request->getFile('image', FILTER_UNSAFE_RAW))
            {
            ...
            if(empty($img->getClientExtension())){
                $image = $this->request->getVar('old_image', FILTER_UNSAFE_RAW);
            } else {
            ...
            if(!empty($this->request->getVar('password'))){
                ...
                $passwordNew = md5($this->request->getVar('password', FILTER_UNSAFE_RAW));
            ...
            $data['user'] = (object)$userData = array(
                ...
                'firstname'   => $this->request->getVar('firstname', FILTER_UNSAFE_RAW),
                'lastname'    => $this->request->getVar('lastname', FILTER_UNSAFE_RAW),
                'email'       => $this->request->getVar('email', FILTER_UNSAFE_RAW),
                'password'    => $passwordNew,
                'about'       => $this->request->getVar('about', FILTER_UNSAFE_RAW),
                'image'       => $image 
            );
            ...
            if ($this->validate($rules, $rules)) {
                ...
                if ($this->common_model->update('admin',$userData, array('id' => $id))) 
                {
                    ...
                    $this->session->set(array(
                        'fullname'   => $this->request->getVar('firstname', FILTER_UNSAFE_RAW). ' ' .$this->request->getVar('lastname', FILTER_UNSAFE_RAW),
                        'email'      => $this->request->getVar('email', FILTER_UNSAFE_RAW),
                        'image'      => (!empty($image)?$image:$this->request->getVar('old_image', FILTER_UNSAFE_RAW))
                    ));
