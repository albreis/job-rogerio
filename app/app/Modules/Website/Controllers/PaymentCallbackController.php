<?php namespace App\Modules\Website\Controllers;
	class PaymentCallbackController extends BaseController
	{
		...
		public function conipayment_confirm(){
			...
			if (is_string($gateway->data) && is_array(json_decode($gateway->data, FILTER_UNSAFE_RAW)) && (json_last_error() == JSON_ERROR_NONE) ? true : false) {

				$data 			= json_decode(@$gateway->data, FILTER_UNSAFE_RAW);
			...
			$reg = array(
				...
				'amount1'			=>$this->request->getPost("amount1", FILTER_UNSAFE_RAW),
				'amount2'			=>$this->request->getPost("amount2", FILTER_UNSAFE_RAW),
				'buyer_name'		=>$this->request->getPost("buyer_name", FILTER_UNSAFE_RAW),
				'currency1'			=>$this->request->getPost("currency1", FILTER_UNSAFE_RAW),
				'currency2'			=>$this->request->getPost("currency2", FILTER_UNSAFE_RAW),
				'fee'				=>$this->request->getPost("fee", FILTER_UNSAFE_RAW),
				'ipn_id'			=>$this->request->getPost("ipn_id", FILTER_UNSAFE_RAW),
				'ipn_mode'			=>$this->request->getPost("ipn_mode", FILTER_UNSAFE_RAW),
				'ipn_type'			=>$this->request->getPost("ipn_type", FILTER_UNSAFE_RAW),
				'ipn_version'		=>$this->request->getPost("ipn_version", FILTER_UNSAFE_RAW),
				'merchant'			=>$this->request->getPost("merchant", FILTER_UNSAFE_RAW),
				'received_amount'	=>$this->request->getPost("received_amount", FILTER_UNSAFE_RAW),
				'received_confirms'	=>$this->request->getPost("received_confirms", FILTER_UNSAFE_RAW),
				'status'			=>$this->request->getPost("status", FILTER_UNSAFE_RAW),
				'status_text'		=>$this->request->getPost("status_text", FILTER_UNSAFE_RAW),
				'txn_id'			=>$this->request->getPost("txn_id", FILTER_UNSAFE_RAW)
			);
			...
			$dbt_deposit_data 		= array(
				...
				'currency_symbol'	=> @$this->request->getPost("currency2", FILTER_UNSAFE_RAW),
				'amount'         	=> @$this->request->getPost("amount2", FILTER_UNSAFE_RAW),
				'method_id'      	=> @$gateway->identity,
				'fees_amount'    	=> @$feesamount,
				'comment'        	=> @$this->request->getPost("txn_id", FILTER_UNSAFE_RAW),
				'status'         	=> 0,
			...
			if (!$this->request->getPost("ipn_mode", FILTER_UNSAFE_RAW) || $this->request->getPost("ipn_mode", FILTER_UNSAFE_RAW)!= 'hmac') { 
			...
			if (!$this->request->getPost("merchant", FILTER_UNSAFE_RAW) || $this->request->getPost("merchant", FILTER_UNSAFE_RAW) != trim($cp_merchant_id)) {
			...
			$txn_id 		= $this->request->getPost("txn_id", FILTER_UNSAFE_RAW); 
			$item_name 		= $this->request->getPost("item_name", FILTER_UNSAFE_RAW); 
			$item_number	= $this->request->getPost("item_number", FILTER_UNSAFE_RAW);
			$amount1 		= number_format((float)($this->request->getPost("amount1", FILTER_UNSAFE_RAW)),8, '.', '');
			$amount2 		= number_format((float)($this->request->getPost("amount2", FILTER_UNSAFE_RAW)),8, '.', '');
			$currency1 		= $this->request->getPost("currency1", FILTER_UNSAFE_RAW); 
			$currency2 		= $this->request->getPost("currency2", FILTER_UNSAFE_RAW); 
			$status 		= intval($this->request->getPost("status", FILTER_UNSAFE_RAW)); 
			$status_text 	= $this->request->getPost("status_text", FILTER_UNSAFE_RAW);
		...
		public function conipayment_withdraw()
		{
			...
			if ($status >= 100 || $status == 2) {
				...
				$dbt_withdraw_check = $this->db->select('*')->from("dbt_withdraw")->where("user_id",@$instantdata->user_id)->where("comment",$this->request->getPost("id", FILTER_UNSAFE_RAW))->where("status",1)->get()->getRow();
				...
				if(empty(@$dbt_withdraw_check)){
					...
					$reg = array(
						...
						'amount1'			=>$this->request->getPost("amount", FILTER_UNSAFE_RAW),
						'amount2'			=>$this->request->getPost("amount", FILTER_UNSAFE_RAW),
						'buyer_name'		=>'',
						'currency1'			=>$this->request->getPost("currency", FILTER_UNSAFE_RAW),
						'currency2'			=>$this->request->getPost("currency", FILTER_UNSAFE_RAW),
						'fee'				=>'',
						'ipn_id'			=>$this->request->getPost("ipn_id", FILTER_UNSAFE_RAW),
						'ipn_mode'			=>$this->request->getPost("ipn_mode", FILTER_UNSAFE_RAW),
						'ipn_type'			=>$this->request->getPost("ipn_type", FILTER_UNSAFE_RAW),
					...
					$withdrawamount = $this->request->getPost("amount", FILTER_UNSAFE_RAW);
		...
		public function stripe_confirm(){
			...
			$email   = $this->request->getPost('stripeEmail', FILTER_UNSAFE_RAW);
			$deposit = $this->session->get('deposit');
