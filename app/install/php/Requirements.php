...
{  
    ...
    { 
        ...
        $directories = [
            ...
            "sql/install.sql",
            '../app/Modules/Gourl/Libraries/gourl/lib/cryptobox.config.php',
            '../vendor/codeigniter4/framework/system/Security/lic.php',
        ]; 
            ...
            if (file_exists('sql/install.sql')) {  
                ...
                if (sizeof($not_writeable) > 0) { 
                    ...
                    if (sizeof($writeable) > 0) { 
                        ...
                        echo "</tr>"; 
                        echo "<tr>";
                            echo "<td colspan='2'><div class='alert alert-info'><strong>Please!</strong> Make sure your root directory, writeable/session, writeable/logs, public/assets/images/captcha this directories have write, install folder has delete, install/flag folder has write permissions.</div></td>";
                        echo "</tr>"; 
                        //show writeable file
                        for ($i = 0; $i < sizeof($writeable); $i++){ 
                            echo "<tr>";
    ...
    public function checkServerRequirement(array $extensions) 
    {
    ...
    //display server requirements
    public function extensionCheck(array $extensions) 
    {
        ...
        if (version_compare(phpversion(), '7.4', '<')) {
            ...
            $fail[] = 'You need<strong> PHP 7.4</strong> (or greater;<strong>Current Version:'.phpversion().')</strong>';
            //set error status for php version
            $fail_status[] = '<span class="label label-danger">Error</span>';
        } else {
            //set success message for php version
            $pass[] ='You have<strong> PHP 7.4</strong> (or greater; <strong>Current Version:'.phpversion().')</strong>';
        ...
        if (!ini_get('safe_mode')) {
            ...
            if (version_compare($match[0], '7.4', '<')) {
                //set error message if mysql version 
                $fail[] = 'You need<strong> MySQL 7.4</strong> (or greater; <strong>Current Version:.'.$match[0].')</strong>';
                //set error status for mysql version
                $fail_status[] = '<span class="label label-danger">Error</span>';
            } else {
                //set success message
                $pass[] ='You have<strong> MySQL 7.4</strong> (or greater; <strong>Current Version:'.$match[0].')</strong>';
                $pass_status[] = '<span class="label label-success">Success</span>';
            }
