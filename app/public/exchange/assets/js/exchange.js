...
"use strict";
    ...
    $.getJSON("https://min-api.cryptocompare.com/data/price?fsym=" + cryptolistfrom + "&tsyms=" + cryptolistto + "&api_key=" + BDTASK.crypto_api(), function(result) {
        ...
        systemlatestPrice(rate);

    });

    //Market coinpair load 
    function systemlatestPrice(rate = 0) {


        $.getJSON(BDTASK.getSiteAction('get-system-latest-price?market=' + market), function(data) {

            let systemLatesPrice = 0.00000000;
            
            $.each(data.marketstreamer, function(index, element) {

                systemLatesPrice = parseFloat(element.last_price).toFixed(8);

                if((element.last_price > 0)){

                    $('#buypricing').val(systemLatesPrice);
                    $('#sellpricing').val(systemLatesPrice);
    ...
    $("body").on("click", ".language-exchange", function(event) {
        event.preventDefault();
